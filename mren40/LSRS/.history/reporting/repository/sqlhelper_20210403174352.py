...
class SqlHelper(object):
    ...
    def get_report8(self):
        self.cursor.execute(
            "SELECT COLS.Category, COLS.`Store Type`, IFNULL(ACT.`Quantity Sold`, 0) FROM"
            ...
            ") AS ACT"
            "ON COLS.Category = ACT.Category AND COLS.`Store Type` = ACT.`Store Type` ORDER BY COLS.Category ASC, COLS.`Store Type` ASC;"
        )
